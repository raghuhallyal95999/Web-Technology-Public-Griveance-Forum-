		...
		if(err){
			...
			for(i in result){
				...
				if(result[i].time_slot=="16:00-18:00")
					slot="slot3";
				if(result[i].catagory=="Electrical Dept Official" ){
				...
				else if(result[i].catagory=="Water Dept Official" ){
...
module.exports={
	assign: function(req,res,con){
		...
		con.query(sql,function(err,result){
			...
			sql="select * from handyman_info where Catagory='Electrical Dept Official' and handyman_id in "+ids;
			con.query(sql,function(err,result){
				if(err!=null){
					console.log(err);
					res.end("false");
					...
					sql="select * from handyman_info where Catagory='Water Dept Official' and handyman_id in "+ids;
					con.query(sql,function(err,result){
						if(err){
							console.log(err);
							res.end("false");
